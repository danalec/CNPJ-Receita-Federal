...
class Estabelecimento(Base):
    ...
    cnpj_basico = Column(String(8), ForeignKey('empresas.cnpj_basico'), primary_key=True)
...
class Socio(Base):
    ...
    qualificacao_socio_codigo = Column(Integer, ForeignKey('qualificacoes_socios.codigo'))
    ...
    qualificacao_representante_legal_codigo = Column(Integer, ForeignKey('qualificacoes_socios.codigo'))
    faixa_etaria = Column(Enum(FaixaEtariaSocioEnum))

    # Relacionamentos
    empresa = relationship("Empresa", back_populates="socios")
    pais = relationship("Pais")
    qualificacao_socio = relationship("QualificacaoSocio", foreign_keys=[qualificacao_socio_codigo])
    qualificacao_representante_legal = relationship("QualificacaoSocio", foreign_keys=[qualificacao_representante_legal_codigo])


if __name__ == '__main__':
    # Cria uma instância do motor do banco de dados (neste caso, SQLite em memória)
